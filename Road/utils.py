...
def valTrackbars():
    ...
    src = np.float32(
        [
            (widthTop / 100, heightTop / 100),
            (1 - (widthTop / 100), heightTop / 100),
...
def sliding_window(img, nwindows=15, margin=50, minpix=1, draw_windows=True):
    ...
    #        if len(good_right_inds) > minpix:
    #            rightx_current = np.int16(np.mean([leftx_current +900, np.mean(nonzerox[good_right_inds])]))
    #        elif len(good_left_inds) > minpix:
    #            rightx_current = np.int16(np.mean([np.mean(nonzerox[good_left_inds]) +900, rightx_current]))
    #        if len(good_left_inds) > minpix:
    #            leftx_current = np.int16(np.mean([rightx_current -900, np.mean(nonzerox[good_left_inds])]))
    #        elif len(good_right_inds) > minpix:
    #            leftx_current = np.int16(np.mean([np.mean(nonzerox[good_right_inds]) -900, leftx_current]))
...
def draw_lanes(img, left_fit, right_fit, frameWidth, frameHeight, src):
    ...
    inv_perspective = inv_perspective_warp(
        color_img, (frameWidth, frameHeight), dst=src
    )
...
def textDisplay(curve, img):
    ...
    cv2.putText(
        img,
        directionText,
        ((img.shape[1] // 2) - 35, (img.shape[0]) - 20),
        font,
        1,
        (0, 200, 200),
        2,
...
def drawLines(img, lane_curve):
    ...
    for x in range(-30, 30):
        ...
        cv2.line(
            img,
            (w * x + int(lane_curve // 100), myHeight - 30),
            (w * x + int(lane_curve // 100), myHeight),
            (0, 0, 255),
            2,
        )
    cv2.line(
    ...
    cv2.line(
        img, (myWidth // 2, myHeight - 50), (myWidth // 2, myHeight), (0, 255, 255), 2
    )
